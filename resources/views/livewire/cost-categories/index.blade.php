<div class="space-y-4">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        @can('create', \App\Models\CostCategory::class)
            <a
                href="{{ route('cost-categories.create') }}"
                wire:navigate
    ...
    <div class="overflow-x-auto rounded-lg border border-slate-200 bg-white shadow-sm">
        <table class="min-w-full divide-y divide-slate-200 text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                    ...
                    <tr wire:key="cc-{{ $category->id }}">
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="flex justify-end gap-3">
                                @can('update', $category)
                                    <a href="{{ route('cost-categories.edit', $category) }}" wire:navigate class="text-blue-600 hover:underline">
                                        Ubah
                                    </a>
                                @endcan
                                @can('delete', $category)
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-4 py-8 text-center text-slate-400">Belum ada kategori biaya.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
