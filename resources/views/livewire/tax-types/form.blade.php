<div class="max-w-2xl">
    <div class="mb-4">
        <a href="{{ route('tax-types.index') }}" wire:navigate class="text-sm text-slate-500 hover:text-slate-700">
            &larr; Kembali ke daftar jenis pajak
        </a>
    </div>
    ...
    <form wire:submit="save" class="space-y-5 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div>
            <label for="rate" class="mb-1 block text-sm font-medium text-slate-700">Tarif (%)</label>
            <input wire:model="rate" id="rate" type="number" step="0.01" class="block w-full max-w-xs rounded-md border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
        ...
        <div class="flex justify-end gap-3 border-t border-slate-100 pt-4">
            <a href="{{ route('tax-types.index') }}" wire:navigate class="rounded-md border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50">
                Batal
            </a>
