        ...
        <table class="min-w-full divide-y divide-slate-200 text-sm">
            <thead class="bg-slate-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left font-medium text-slate-500">Kontak</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($clients as $client)
                    <tr wire:key="client-{{ $client->id }}">
                        ...
                        <td class="px-4 py-3 text-slate-500">{{ $client->email ?? $client->phone ?? '—' }}</td>
                        <td class="px-4 py-3">
                            <x-status-badge :active="$client->is_active" />
                        </td>
                        <td class="px-4 py-3 text-right">
                            <div class="flex justify-end gap-3">
                                @can('update', $client)
                                    <a href="{{ route('clients.edit', $client) }}" wire:navigate class="text-blue-600 hover:underline">
                                        Ubah
                                    </a>
                                @endcan
                                @can('delete', $client)
                                    <button
                                        ...
                                        wire:confirm="Hapus klien \"{{ $client->name }}\"?"
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-4 py-8 text-center text-slate-400">Belum ada klien.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
